<?php
  ...
  if (!is_set($_POST['changed_nickname']) || !is_set($_POST['username']) || !is_set($_POST['password'])) {
    header('Location:alter_nickname.php?errCode=1');
    die('資料不齊全');
  }
  ...
  if (!($username === $_POST['username'])) {
    // 帳號錯誤（type 2 error）
    header('Location:alter_nickname.php?errCode=2');
    die('帳號錯誤');
  }
  ...
  if (password_verify($password,  getdata_from_username($username)['password'])) {
    ...
    header('Location:alter_nickname.php?errCode=3');
    die('密碼錯誤');    
  }
?>