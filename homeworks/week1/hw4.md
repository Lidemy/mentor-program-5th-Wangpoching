...
```
現在 Github 上的 repository 裡就可以看到最新版本的論文。
...
除此之外 reset 有兩個模式可以選擇，分別是 **Hard** 以及 **Soft**, 預設是使用 soft 模式，Hard 模式除了回到特定的 commit 以外還會將目前的 working tree 給清空，但是 soft 模式雖然也會到特定的 commit 但會保留目前的 working tree。
...
但是因為不是自己的專案，所以在 clone 下來的過程中是無法將檔案修改以後 push 回去。
